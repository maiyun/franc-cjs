import * as franc from './index';

console.log(franc.francAll('Сегодня погода такая хорошая, ты что думаешь? Я думаю, что просто так.', {
    'only': ['cmn', 'rus', 'jpn']
}));

console.log(franc.franc('O tempo está realmente bom hoje, o que você acha? Eu acho que é isso.'));

console.log(franc.franc('The weather is really nice today, don\'t you think? I think it\'s just perfect.'));

console.log(franc.franc('今日の天気はとてもいいですね、どう思いますか？私はこれがちょうどいいと思います。'));

console.log(franc.franc('今天的天气真好，你觉得呢？我觉得就是这样。'));

